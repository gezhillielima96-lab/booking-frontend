...
function Admin() {
  ...
  // States për SHTIM
  const [newProp, setNewProp] = useState({ emri_prones: '', lokacioni: '', cmimi: '', image: null });
  const [newRoom, setNewRoom] = useState({ property_id: '', tipi: '', cmimi: '', kapaciteti: '', images: [] }); 
  
  // States për EDITIM
  const [editPropData, setEditPropData] = useState({ id: '', emri_prones: '', lokacioni: '', cmimi: '', image: null });
  ...
  const handleAddProperty = async () => {
    ...
    formData.append('cmimi', newProp.cmimi);
  ...
  const handleAddRoom = async () => {
    ...
    try {
      ...
      if(selectedProperty) handleSelectProperty(selectedProperty);
      alert("Dhoma u shtua!");
    } catch (err) { alert("Gabim gjatë shtimit të dhomës!"); }
  };
  ...
  const handleUpdateProperty = async () => {
    ...
    formData.append('cmimi', editPropData.cmimi);
  ...
  const handleUpdateRoom = async () => {
    ...
    if (editRoomData.images) {
        Array.from(editRoomData.images).forEach(file => {
            formData.append('images', file);
        });
    }
  ...
  return (
    <Container fluid className="py-4">
      ...
      <Table striped bordered hover responsive className="mb-5 shadow-sm bg-white">
        <thead className="table-dark">
          <tr><th>Klienti</th><th>Prona</th><th>Total</th><th>Statusi</th><th>Veprime</th></tr>
        </thead>
        <tbody>
          {data.map((item, index) => (
      ...
      <h4 className="fw-bold text-secondary mb-3">Hotelet (Kliko rreshtin për dhomat)</h4>
      <Table hover bordered responsive className="shadow-sm bg-white">
        <thead className="table-secondary">
          <tr><th>Foto</th><th>Emri</th><th>Lokacioni</th><th>Çmimi</th><th>Veprime</th></tr>
        </thead>
        <tbody>
          {properties.map((p, index) => (
            <tr key={`prop-${p.id}-${index}`} onClick={() => handleSelectProperty(p)} style={{ cursor: 'pointer' }}>
              <td>
                <img 
                    src={p.foto ? `http://localhost:5000${p.foto}` : 'https://via.placeholder.com/50'} 
                    alt="hotel" 
              ...
              {rooms.map((r, index) => {
                ...
                return (
                    <tr key={`room-${r.id}-${index}`}>
                      <td>
                        <img 
                            src={firstImg ? `http://localhost:5000${firstImg}` : 'https://via.placeholder.com/50'} 
                            alt="room" 
                            style={{ width: '50px', height: '40px', objectFit: 'cover', borderRadius: '4px' }}
                        />
                      </td>
      ...
      <Modal show={showAddProp} onHide={() => setShowAddProp(false)} centered>
        ...
        <Modal.Body>
          ...
          <Form.Label>Çmimi (Starting From €)</Form.Label>
          <Form.Control className="mb-3" type="number" onChange={e => setNewProp({...newProp, cmimi: e.target.value})} />
          <Form.Label>Foto e Hotelit</Form.Label>
          <Form.Control type="file" onChange={e => setNewProp({...newProp, image: e.target.files[0]})} />
        </Modal.Body>
        <Modal.Footer><Button variant="success" onClick={handleAddProperty}>Ruaj Pronën</Button></Modal.Footer>
      </Modal>
      ...
      <Modal show={showEditProp} onHide={() => setShowEditProp(false)} centered>
        ...
        <Modal.Body>
          ...
          <Form.Label>Çmimi i ri</Form.Label>
          <Form.Control className="mb-3" type="number" value={editPropData.cmimi} onChange={e => setEditPropData({...editPropData, cmimi: e.target.value})} />
          <Form.Label>Ndrysho Foton</Form.Label>
          <Form.Control type="file" onChange={e => setEditPropData({...editPropData, image: e.target.files[0]})} />
        </Modal.Body>
        <Modal.Footer><Button variant="warning" onClick={handleUpdateProperty}>Ruaj Ndryshimet</Button></Modal.Footer>
      </Modal>

      {/* MODAL SHTO DHOMË */}
      <Modal show={showAddRoom} onHide={() => setShowAddRoom(false)} centered>
        <Modal.Header closeButton className="bg-primary text-white"><strong>Shto Dhomë të Re</strong></Modal.Header>
        <Modal.Body>
          ...
          <Form.Label>Emri i Dhomës</Form.Label>
          ...
          <Form.Label>Foto të Dhomës (Zgjidhni disa)</Form.Label>
          <Form.Control type="file" multiple onChange={e => setNewRoom({...newRoom, images: e.target.files})} />
        </Modal.Body>
        <Modal.Footer><Button variant="primary" onClick={handleAddRoom}>Ruaj Dhomën</Button></Modal.Footer>
      </Modal>
      ...
      <Modal show={showEditRoom} onHide={() => setShowEditRoom(false)} centered>
        ...
        <Modal.Body>
          <Form.Label>Prona (Hoteli)</Form.Label>
          <Form.Select className="mb-2" value={editRoomData.property_id} onChange={e => setEditRoomData({...editRoomData, property_id: e.target.value})}>
            {properties.map(p => <option key={p.id} value={p.id}>{p.emri_prones}</option>)}
          </Form.Select>
          <Form.Label>Emri i Dhomës</Form.Label>
          ...
          <Form.Label>Ndrysho/Shto Foto të Reja</Form.Label>
          <Form.Control type="file" multiple onChange={e => setEditRoomData({...editRoomData, images: e.target.files})} />
        </Modal.Body>
        <Modal.Footer><Button variant="primary" onClick={handleUpdateRoom}>Ruaj Ndryshimet</Button></Modal.Footer>
      </Modal>
